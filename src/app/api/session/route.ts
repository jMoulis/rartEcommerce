...
export async function POST(request: NextRequest) {
  try {
    ...
    const formattedOccurrences = occurrences.map(formatDateInfo);

    return NextResponse.json<APIResponse<IOccurence[]>>({ error: null, success: true, data: formattedOccurrences });
  } catch (error: any) {
